...
Basically, it allows different websites, computer devices, and data talk to each other.
...
### Adding the `Papa Parse` JavaScript

Instead of linking to `papa parse` directly online like we did with Leaflet, we want to bring the library of sweet pastries into our site. To do so, we will create a new file inside our `js` folder, just like we do with our `init.js`!

Let's name it `papaparse.min.js`. You can name it anything really, but it is helpful to use the official naming incase you need to remind yourself what it was called later.

Now, go to the `papaparse` the GitHub repo:

[https://github.com/mholt/PapaParse/blob/master/papaparse.min.js](https://github.com/mholt/PapaParse/blob/master/papaparse.min.js)

Click ==**Raw**== or select the code.

Then copy and paste it into our `papaparse.min.js` file!

You can also copy it from here:
```js title="js/papaparse.min.js"
/* @license
Papa Parse
v5.3.2
...
With the library ready to go, let's add it to our HTML file, right after the `Leaflet.js` and before the end of the `<head>`.
...
Next we will go into our `init.js` and change out the `fetch` in the `loadData()` function to the following:
